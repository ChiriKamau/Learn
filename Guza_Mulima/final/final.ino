...
// Define your timezone in hours (e.g., +2 for SAST - South African Standard Time)
#define TIME_ZONE_OFFSET 3  // Change this to match your local timezone
...
void updateDisplay(float dht_temp, float dht_humid, float bmp_temp, float pressure, float altitude, int hour, int minute, int second, double lat, double lng, bool hasGpsFix);
...
void setup() {
  ...
  M5.Lcd.setCursor(20, 100); M5.Lcd.print("Initializing GPS...");
  M5.Lcd.setCursor(20, 130); M5.Lcd.print("Time Zone: UTC+");
  M5.Lcd.print(TIME_ZONE_OFFSET);
  M5.Lcd.setCursor(20, 180); M5.Lcd.print("Press any button");
  M5.Lcd.setCursor(20, 210); M5.Lcd.print("to view data");
...
void loop() {
  ...
  while (GPSserial.available()) {
    if (gps.encode(GPSserial.read())) {
      // Check if we have a new GPS fix
  ...
  // If it's been more than 2 minutes without a GPS fix, consider it lost
  if (millis() - lastGpsFixTime > 120000) {
    hasGpsFix = false;
  }
  ...
  if (gps.time.isValid()) {
    hour = (gps.time.hour() + TIME_ZONE_OFFSET) % 24; // Apply timezone offset
  ...
  if (M5.BtnA.wasPressed() || M5.BtnB.wasPressed() || M5.BtnC.wasPressed()) {
    ...
    updateDisplay(dht_temp, dht_humid, bmp_temp, bmp_event.pressure, altitude, hour, minute, second, lat, lng, hasGpsFix);
  }
...
void updateDisplay(float dht_temp, float dht_humid, float bmp_temp, float pressure, float altitude, int hour, int minute, int second, double lat, double lng, bool hasGpsFix) {
  ...
  // Display time with different color based on GPS fix status
  if (hasGpsFix) {
    M5.Lcd.setTextColor(GREEN);
  } else {
    M5.Lcd.setTextColor(YELLOW); // Yellow if no GPS fix
  }
  ...
  // Display GPS info with different color based on fix status
  if (hasGpsFix) {
    M5.Lcd.setTextColor(WHITE);
  } else {
    M5.Lcd.setTextColor(RED); // Red if no GPS fix
  }
  M5.Lcd.setCursor(10, 200); M5.Lcd.setTextSize(2);
